...
class SLUTagging(nn.Module):
    ...
    def forward(self, batch):
        ...
        # else:
        #     loss_2 = self.crf.neg_log_likelihood_loss(tag_output, tag_mask.bool(), tag_ids)

        # loss = loss_1 + loss_2 * self.crfloss_lamb
...
class TaggingFNNDecoder(nn.Module):
    ...
    def __init__(self, input_size, num_tags, pad_id):
        ...
        self.output_layer = nn.Linear(input_size, num_tags+2)
    ...
    def forward(self, hiddens, mask, labels=None):
        ...
        ori_logits = logits
        # print(logits.shape)
        logits += (1 - mask).unsqueeze(-1).repeat(1, 1, self.num_tags+2) * -1e32
        # prob = torch.softmax(logits, dim=-1)
        if labels is not None:
            loss = self.loss_fct(logits.view(-1, logits.shape[-1]), labels.view(-1))
            return ori_logits, loss
        return (ori_logits, )
