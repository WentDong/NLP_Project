...
class SLUTagging(nn.Module):
    ...
    def __init__(self, config):
        ...
        # CRF
        args_crf = dict({'target_size': config.num_tags, "use_cuda": True, "average_batch": True})
        self.crf = CRF(**args_crf)
        # self.crfloss_lamb = 0.012
    ...
    def forward(self, batch):
        ...
        inputs = [self.tokenizer(batch.utt[i], return_tensors="pt").to("cuda:0") for i in range(len(batch.utt))]
        out = [self.model(**inputs[i])['last_hidden_state'][:,1:-1] for i in range(len(batch.utt))]
        out = [nn.functional.pad(out[i], pad=(0,0,0,max_length-lengths[i]), value=0) for i in range(len(batch.utt))]
        embed = torch.concat(out)
        ...
        packed_inputs = packed_inputs.to("cuda:0")
